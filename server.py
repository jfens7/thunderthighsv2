...
try:
    # Delays the first background sync by 10 minutes so it doesn't block your startup
...
def get_players(): 
    if db:
        return jsonify(list(db.get_all_players().keys())) 
    return jsonify([])
...
def get_rankings(season, division): 
    if db:
        return jsonify(db.get_division_rankings(season, division, request.args.get('week', 'Latest')))
    return jsonify([])
...
def top_donors(): 
    if db:
        return jsonify(db.get_top_donors())
    return jsonify([])

@app.route('/api/notices')
def get_notices(): 
    if db:
        return jsonify(db.get_notices())
    return jsonify([])
...
def search_history(): 
    if db:
        return jsonify(db.admin_search_history(request.args.get('q', ''), source_filter=request.args.get('source', 'All')))
    return jsonify([])
...
def manual_match(): 
    if db:
        return jsonify({"success": db.admin_add_manual_match(request.json.get('p1'), request.json.get('p2'), request.json.get('date'), request.json.get('scores'), session.get('admin_email'))})
    return jsonify({"success": False})
...
def update_match(): 
    if db:
        return jsonify({"success": db.admin_update_historical_match(request.json.get('p1'), request.json.get('p2'), request.json.get('date'), request.json.get('s1'), request.json.get('s2'), request.json.get('new_date'), session.get('admin_email'))})
    return jsonify({"success": False})
...
def bulk_fix_date(): 
    if db:
        return jsonify({"success": db.admin_bulk_fix_date(request.json.get('season'), request.json.get('division'), request.json.get('week'), request.json.get('date'), session.get('admin_email'))})
    return jsonify({"success": False})
...
def override_deltas(): 
    if db:
        return jsonify({"success": db.admin_override_match_deltas(request.json.get('match_id'), request.json.get('p1_delta'), request.json.get('p2_delta'), session.get('admin_email'))})
    return jsonify({"success": False})
...
def set_rating_scales(): 
    if db:
        return jsonify({"success": db.admin_set_rating_scales(request.json.get('k_win'), request.json.get('k_loss'), session.get('admin_email'))})
    return jsonify({"success": False})
...
def get_rating_scales(): 
    if db:
        return jsonify({"k_win": db.k_win, "k_loss": db.k_loss})
...
def chaos_vote(): 
    if db:
        return jsonify({"success": db.admin_vote_chaos(request.json.get('weeks', []), session.get('admin_email'))})
    return jsonify({"success": False})
...
def upload_schedule():
    if not db: 
        return jsonify({"success": False, "error": "DB Offline"})
...
def get_player_directory(): 
    if db:
        return jsonify(db.admin_get_player_directory())
    return jsonify([])
...
def glicko_calc(): 
    if db:
        return jsonify({"success": True, "data": db.admin_glicko_math(request.json.get('p1'), request.json.get('p2'), request.json.get('s1'), request.json.get('s2'))})
    return jsonify({"success": False})
...
def add_notice(): 
    if db:
        return jsonify({"success": db.admin_add_notice(request.json.get('title'), request.json.get('message'), request.json.get('type'), session.get('admin_email'))})
    return jsonify({"success": False})
...
def admin_users(): 
    if db:
        return jsonify(db.get_admin_users())
    return jsonify([])
...
def force_refresh():
    if db: 
        db.refresh_data()
        db._log_audit(session.get('admin_email', 'Unknown'), 'FORCE_REFRESH', "Manually triggered a database sync.", {})
...
def hub_register(): 
    if db:
        return jsonify(db.register_player_account(request.json.get('name'), request.json.get('dob'), request.json.get('email'), request.json.get('uid'), request.json.get('estimated_rating')))
    return jsonify({"success": False})

@app.route('/api/hub/forum', methods=['GET'])
def get_forum(): 
    if db:
        return jsonify(db.get_community_feed())
    return jsonify([])

@app.route('/api/hub/post', methods=['POST'])
def make_post(): 
    if db:
        return jsonify({"success": db.create_community_post(request.json.get('uid'), request.json.get('name'), request.json.get('content'), request.json.get('type', 'General'), request.json.get('image_url'), request.json.get('poll_options'))})
    return jsonify({"success": False})

@app.route('/api/hub/vote', methods=['POST'])
def vote_poll(): 
    if db:
        return jsonify({"success": db.vote_community_poll(request.json.get('post_id'), request.json.get('option'), request.json.get('uid'))})
    return jsonify({"success": False})

@app.route('/api/hub/schedule/<player_name>', methods=['GET'])
def get_schedule(player_name): 
    if db:
        return jsonify(db.get_player_upcoming_schedule(player_name))
    return jsonify([])

@app.route('/api/hub/tournament/update_cart', methods=['POST'])
def update_cart(): 
    if db:
        return jsonify(db.process_tournament_cart(request.json.get('uid'), request.json.get('tournament_id'), request.json.get('events'), request.json.get('total')))
    return jsonify({"success": False})
